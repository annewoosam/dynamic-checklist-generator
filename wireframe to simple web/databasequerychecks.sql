...
CREATE TABLE checklisttemplatequestions (
                     checklisttemplatequestions_id SERIAL PRIMARY KEY,
...
INSERT INTO checklisttemplatequestions (
                     checklist_id,
                     template_name,
                     question_number
                     )
VALUES
(1,'compliance testing',1),
(1,'compliance testing',2),
(1,'compliance testing',3),
(1,'compliance testing',4),
(1,'compliance testing',5),
(1,'compliance testing',6),
(1,'compliance testing',7),
(1,'compliance testing',8),
(1,'compliance testing',9);
-- Tests
...
-- Sample from https://www.geeksforgeeks.org/joining-three-tables-sql/
...
-- select yes_text from template_questions t inner join checklist c on
-- t.template_name=c.template_name inner join answers a on a.checklist_id=c.checklist_id
-- WHERE a.answer='y';

-- select question, yes_text from template_questions t  join checklist c on
-- t.template_name=c.template_name WHERE c.checklist_id=1 AND join answers a on a.checklist_id=c.checklist_id WHERE a.answer='y';

-- SELECT answer, t.yes_text FROM answers WHERE answer='y' and checklist_id=1 JOIN template_questions t on t.template_name=c.template_name;
...
-- ids of plans that passed prep stage
-- select checklist_id from notifications where date_sent_to_review is not null;
-- numbers of plans that passed prep stage
-- for all checklists
-- select count(checklist_id) from notifications where date_sent_to_review is not null;
-- that are compliance testing
-- select notifications.checklist_id, checklist.template_name from notifications JOIN checklist ON checklist.checklist_id=notifications.checklist_id where notifications.date_sent_to_review is not null AND checklist.checklist_name='compliance_testing';
-- select count(notifications.checklist_id) from notifications JOIN checklist ON checklist.checklist_id=notifications.checklist_id where notifications.date_sent_to_review is not null AND checklist.template_name='compliance testing';
